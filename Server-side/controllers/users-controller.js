...
const createUser = (req, res, next) => {
    ...
    const hasUser = USERS.find(u=>{ return u.email === email})

    if(hasUser){
        throw new HttpError("Couldn't create user, user with same ID already exists",422);
    }
    ...
    const newUser = {
      ...
      email: email,
      password: password,
      image: "https://cdn.business2community.com/wp-content/uploads/2014/04/profile-picture-300x300.jpg",
      places : 0
    };
...
const userLogin = (req, res, next) => {
    ...
    if(!identifiedUser || identifiedUser.password != password){
        throw new HttpError("Could not identify user, credentials seems to be wrong");
    }else{
